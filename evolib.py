...
Contains classes defining a Individual, Population, Historical Population
"""
...
class Individual:
    ...
    def __init__(self, problem_instance: Type[Problem], genome=None, children=[], **kwargs):
        self.args = kwargs
        ...
        self.fitness = None
        self.id = Individual.next_id  # Create a unique ID for each individual
        self.age = 0
        Individual.next_id += 1
        HistoricalPopulation.historical_individuals[self.id] = self

    def problem(self) -> Type[Problem]:
    ...
    def mutate(self) -> Self:
        new_individual = type(self)(
            self.problem_instance, self.problem_instance.mutate(self.genome), **self.args
        )
    ...
    def recombine(self, other):
        new_individual = type(self)(
            ...
            self.problem_instance.recombine(self.genome, other.genome),
            **self.args
        )
...
class Population:
    def __init__(self, problem_instance: Type[Problem], individuals=None, **kwargs):
        self.args = kwargs
        self.problem_instance = problem_instance
        if type(individuals) == list:
            self.individual_class = kwargs.get(
                "individual_class", (len(individuals) > 0) and type(individuals[0]) or Individual
            )
    ...
    def evaluate(self):
        ...
        self.individuals.sort(
            reverse=self.problem_instance.maximizing(),
            key=lambda individual: individual.evaluate(current_population),
        )
        return self
    ...
    def mutate(self):
        ...
        self.individuals += [
            self.invoke_mutation(individual)
            for individual in self.individuals
            if randb(self.mut_rate)
        ]
        return self
    ...
    def recombine(self):
        self.evaluate()
        self.individuals += [
            self.invoke_recombination(individual)
            for individual in self.individuals
            if randb(self.rec_rate)
        ]
        return self
...
class HistoricalPopulation(Population):
    ...
    def compute_pf(self, individual, depth=0):
        ...
        if len(individual.children) == 0:
            ...
            for child_id in individual.children:
                children_pf += [
                    self.compute_pf(
                        HistoricalPopulation.historical_individuals[child_id], depth + 1
                    )
                ]
    ...
    def compute_opf(self, individual, depth=0):
        ...
        if len(individual.children) == 0:
            ...
            opt_productive_fitness = (
                self.problem_instance.maximizing() and max(children_pf) or min(children_pf)
            )
    ...
    def is_ancestor(self, ancestor, descendant):
        if str(descendant.id) + " " + str(ancestor.id) in HistoricalPopulation.historical_ancestors:
            return HistoricalPopulation.historical_ancestors[
                str(descendant.id) + " " + str(ancestor.id)
            ]
        if descendant.id in ancestor.children:
            ...
            for child_id in ancestor.children:
                if self.is_ancestor(
                    HistoricalPopulation.historical_individuals[child_id], descendant
                ):
                    HistoricalPopulation.historical_ancestors[
                        str(descendant.id) + " " + str(ancestor.id)
                    ] = True
                    return True
                HistoricalPopulation.historical_ancestors[
    ...
    def filter_descendants(self, ancestor):
        return [
            individual for individual in self.individuals if self.is_ancestor(ancestor, individual)
        ]
    ...
    def best_pf(self):
        ...
        for individual in self.individuals:
            ...
            pfs += [pf]
            if (self.problem_instance.maximizing() and pf > best_pf) or (
                not self.problem_instance.maximizing() and pf < best_pf
            ):
                best_pf = pf
                best = individual
        return best_pf
    ...
    def best_opf(self):
        ...
        for individual in self.individuals:
            ...
            pfs += [pf]
            if (
                self.problem_instance.maximizing()
                and pf > best_pf
                or not self.problem_instance.maximizing()
                and pf < best_pf
            ):
                best_pf = pf
                best = individual
