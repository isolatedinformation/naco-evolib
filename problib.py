...
class FloatsProblem(VectorProblem):
    ...
    def mutate(self, candidate, **kwargs):
        ...
        if hasattr(self, "mutation_continuous") and self.mutation_continuous:
            strength = (
                hasattr(self, "mutation_strength")
                and self.mutation_strength
                or kwargs.get("strength", 0.1)
            )
            variation = [
                randf(min=strength * self.min(d), max=strength * self.max(d)) for d in self.dims()
            ]
            candidate = [
                np.clip(variation + candidate[d], self.min(d), self.max(d)) for d in self.dims()
            ]
        else:
            ...
            strength = (
                hasattr(self, "mutation_strength")
                and self.mutation_strength
                or kwargs.get("strength", 0.1 * abs(self.min(d) - self.max(d)))
            )
...
class SchwefelProblem(FloatsProblem):
    ...
    def evaluate(self, candidate):
        return 418.9828872724339 * self.dimensionality - sum(
            x * math.sin(math.sqrt(abs(x))) for x in candidate
        )
...
class NormalizedSchwefelProblem(SchwefelProblem):
    def evaluate(self, candidate):
        return (
            418.9828872724339 * self.dimensionality
            - sum(x * math.sin(math.sqrt(abs(x))) for x in candidate)
        ) / 4000.0
...
class RoomWalk(FloatsProblem):
    ...
    def evaluate(self, candidate):
        ...
        for i in range(0, len(candidate), 2):
            ...
            if (
                next_position[0] < 0.0
                or next_position[0] > 1.0
                or next_position[1] < 0.0
                or next_position[1] > 1.0
            ):
                current_reward += self.penalty
            elif (
                next_position[0] > self.obstacle[0][0]
                and next_position[0] < self.obstacle[1][0]
                and next_position[1] > self.obstacle[0][1]
                and next_position[1] < self.obstacle[1][1]
            ):
                current_reward += self.penalty
            else:
                current_position = next_position
                if (
                    current_position[0] > self.target_area[0][0]
                    and current_position[0] < self.target_area[1][0]
                    and current_position[1] > self.target_area[0][1]
                    and current_position[1] < self.target_area[1][1]
                ):
...
class FactoryRobotRoute(VectorProblem):
    ...
    def __init__(self, **kwargs):
        ...
        if type(self.stations) == int:
            self.stations = {
                task: [
                    (randf(max=self.area_size), randf(max=self.area_size))
    ...
    def evaluate(self, candidate):
        ...
        for target_position in path:
            if self.manhattan_distance:
                total_cost += abs(current_position[0] - target_position[0]) + abs(
    ...
    def __init__(self, **kwargs):
        ...
        if type(self.stations) == int:
            self.stations = {
                task: [
                    [
                        randf(min=self.min_cost, max=self.max_cost)
                        for other_station in range(self.stations)
                    ]
                    for _ in range(self.stations)
                ]
                for task in self.tasks
