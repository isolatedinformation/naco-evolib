"""
Experiments Library:
#TODO: Write about this is in detail. 
"""
...
class AbstractExperiment:
    ...
    def experiment_filename(self):
        ...
        return (
            RESULT_FILES_DIR
            + os.path.splitext(os.path.basename(main.__file__))[0]
            + "."
            + str(self.real_time)
        )
...
class ParameterExperiment(AbstractExperiment):
    ...
    def perform(self, **kwargs):
        ...
        if show_progress:
            print(
                ...
                + str(self.parameter)
                + " for "
                + args.get("label", "unlabeled setup")
                + " with "
                + str(args.get("num_runs", "an unknown amount of"))
                + " runs each"
            )
        ...
        for s in settings_iterator:
            ...
            current_results.append(
                [base_experiment.results[-1][t][-1] for t, _ in enumerate(self.tests)]
            )
    ...
    def plot(self, **kwargs):
        ...
        for t, test in enumerate(self.tests):
            ...
            for e, evolution in enumerate(self.evolutions):
                ...
                if args.get("plot_log", False):
                    plot = plotter.semilogy(
                        self.settings,
                        [self.results[e][s][t] for s, setting in enumerate(self.settings)],
                        **evolution_args
                    )[0]
                else:
                    plot = plotter.plot(
            ...
            if subplotting:
                plotter.set(
                    xlabel=str(style_args.get("xlabel", self.parameter)),
                    ylabel=str(
                        style_args.get("ylabel", hasattr(test, "name") and test.name or "result")
                    ),
                )
            else:
                plt.ylabel(
                    str(style_args.get("ylabel", hasattr(test, "name") and test.name or "result"))
                )
        ...
        if (not figuring) and args.get("save_file", False):
            ...
            with open(
                self.experiment_filename() + ".pplot" + str(self.plotted) + ".txt", "w"
            ) as text_file:
...
class Experiment:
    ...
    def perform(self, **kwargs):
        ...
        save_populations = args.get("save_populations", True)
        folding = kwargs.get("folding", lambda runs: sum(runs) / float(len(runs)))
        result = [
            [[None for _ in range(0, num_runs)] for _ in range(0, num_gens)] for _ in self.tests
        ]
        populations = []
        run_iterator = range(0, num_runs)
        if show_progress:
            print(
                "Performing " + str(num_runs) + " runs of " + args.get("label", "unlabeled setup")
            )
        ...
        for run in run_iterator:
            ...
            for gen in range(0, num_gens):
                ...
                for t, test in enumerate(self.tests):
                    if test.is_live():
                        if test.has_context():
                            result[t][gen][run] = test(
                                pop, dict(current_run=run, current_gen=gen, all_populations=pops)
                            )
                        else:
                            result[t][gen][run] = test.run(pop)
            for gen in range(0, num_gens):
                for t, test in enumerate(self.tests):
                    if not test.is_live():
                        if test.has_context():
                            result[t][gen][run] = test(
                                pops[gen],
                                dict(current_run=run, current_gen=gen, all_populations=pops),
                            )
                        else:
    ...
    def plot(self, **kwargs):
        ...
        for t, test in enumerate(self.tests):
            ...
            for e, evolution in enumerate(self.evolutions):
                ...
                evolution_args.update({"label": evolution.get("label", "unkown setup")})
                if correlate_with:
                    evolution_args.update(
                        {
                            "label": str(evolution.get("label", "unkown setup"))
                            + " corr. "
                            + str(
                                pearsonr(
                                    self.results[e][t],
                                    self.results[e][self.tests.index(correlate_with)],
                                )[0]
                            )
                        }
                    )
                ...
                if args.get("plot_std", False):
                    stds = [
                        np.std(self.unfolded_results[e][t][gen])
                        for gen in range(0, len(self.results[e][t]))
                    ]
                    ...
                    if args.get("plot_log", False):
                        plotter.semilogy(
                            time,
                            upper(self.results[e][t], stds),
                            lower(self.results[e][t], stds),
                            alpha=0.1,
                            **evolution_args
                        )
                    else:
            ...
            if args.get("plot_baseline", False) and self.problem_instance.best_result() != None:
                ...
                baseline_args.update(
                    {"label": kwargs.get("baseline_label", "best possible result")}
                )
            ...
            if subplotting:
                plotter.set(
                    xlabel=str(style_args.get("xlabel", "time")),
                    ylabel=str(
                        style_args.get("ylabel", hasattr(test, "name") and test.name or "result")
                    ),
                )
        ...
        if (not figuring) and args.get("save_file", False):
            ...
            with open(
                self.experiment_filename() + ".plot" + str(self.plotted) + ".txt", "w"
            ) as text_file:
                print(str(data), file=text_file)
        if not args.get("no_show", False):
            plt.show()
    ...
    def print_verdict(self, **kwargs):
        ...
        description = ""
        description += (
            "problem:                 " + str(self.problem_instance.__class__.__name__) + "\n"
        )
        ...
        for t, test in enumerate(self.tests):
            ...
            for e, evolution in enumerate(self.evolutions):
                ...
                if len(self.populations) > e and len(self.populations[e]) == 1:
                    description += (
                        "        best individual: " + str(self.populations[e][0].best()) + "\n"
                    )
        ...
        if not len(self.populations) > 0:
            ...
            for e, evolution in enumerate(self.evolutions):
                ...
                for run, population in enumerate(randsel(self.populations[e], amount)):
                    ...
                    description += (
                        "        ... with problem fitness:  \t"
                        + str(population.best().compute_problem_fitness())
                        + "\n"
                    )
        if args.get("save_file", False):
