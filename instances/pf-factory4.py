...
exp = Experiment(NonEuclideanFactoryRobotRoute(tasks=12, stations=5),
...
# exp.perform(population_class=EnsemblePopulation, individual_class=Individual, subpops=2, style=dict(color='g'), label='ensemble')
exp.perform(individual_class=NormalizedHammingDiversityIndividual,
    diversity_weight=0.4,
    style=dict(color='b', ls='-'),
    label='hamming diverse')
exp.perform(individual_class=FitnessSharingIndividual,
    alpha=2.0,
    style=dict(color='orange'),
    label='fitness sharing')
exp.perform(individual_class=InheritedFitnessIndividual,
    inheritance_weight = 0.5,
    style=dict(color='m'),
    label='inherited fitness')
...
exp.plot(show_legend=False, plot_log=False)
