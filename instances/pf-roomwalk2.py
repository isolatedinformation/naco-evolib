...
opf_for = lambda individual, con: con["all_populations"][-1].compute_opf_here(individual) or 0.0
#
fpf = lambda pop, con: avgex([opf_for(individual, con) for individual in pop.individuals])
tests += [ContextTest("opf").with_function(fpf).with_live(False)]
...
afopf = lambda pop, con: avg([abs(individual.get_fitness()             - opf_for(individual, con)) for individual in pop.individuals])
tests += [ContextTest("|af - opf|").with_function(afopf).with_live(False)]
