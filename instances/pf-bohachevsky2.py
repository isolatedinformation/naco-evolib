...
tests += [
    Test("secondary fitness")
    .with_function(lambda pop: pop.best().compute_secondary_fitness(pop))
    .with_live(True)
]
tests += [
    Test("augmented fitness").with_function(lambda pop: pop.best().evaluate(pop)).with_live(True)
]
...
f_increase = (
    lambda pop, context: pop.best_f()
    - context["all_populations"][context["current_gen"] - 1].best_f()
)
pf_increase = (
    lambda pop, context: pop.best_pf()
    - context["all_populations"][context["current_gen"] - 1].best_pf()
)
...
final_descendants = lambda pop, con: avg(
    [
        len(con["all_populations"][-1].filter_descendants(individual))
...
def get_pop(con, offset=0):
    return con["all_populations"][
        max(0, min(con["current_gen"] + offset, len(con["all_populations"]) - 1))
    ]


pf_for = (
    lambda individual, con, x: con["all_populations"][
        min(con["current_gen"] + x, len(con["all_populations"]) - 1)
    ].compute_pf_here(individual)
    or worst
)
...
fpf_incr = lambda pop, con: fpf(pop, con) - fpf(
    con["all_populations"][max(con["current_gen"] - 1, 0)], con
)
...
ffpf = lambda pop, con: avg(
    [
        abs(individual.compute_problem_fitness() - fpf_for(individual, con))
        for individual in pop.individuals
    ]
)
affpf = lambda pop, con: avg(
    [abs(individual.get_fitness() - fpf_for(individual, con)) for individual in pop.individuals]
)
...
tests += [
    ContextTest("delta |af - fpf|")
    .with_function(lambda pop, con: affpf(get_pop(con, -1), con) - affpf(pop, con))
    .with_live(False)
]
...
problem_size = 8
exp = Experiment(
    NormalizedBohachevskyProblem(problem_size),
...
exp.perform(
    individual_class=NormalizedManhattanDiversityIndividual,
